...
while running do
	...
	for i=1,c1,6 do
		...
		runner.far = z-200
		runner.near = 400	
		-- we limit the Y dimension to focus on upper body
		-- pcall is needed because if y==Y*0.7 we'll error
		local ok,c2 = pcall(runner,b2,x,X,y,Y*0.7)
		
		-- If we find some blobs, we'll examine them further
		if ok and c2 > 0 then 
			local j
			for j = 1,c2,6 do
				local x,y,X,Y,z,l = b2(j,6)
				
				-- Let's clear the noise a bit with simple rules
		        local div = (X-x)/(Y-y)
		        if  div < 3 and div > 0.3 then
					
					-- Ok this is probably an arm, let's paint it
					rect.x,rect.y,rect.w,rect.h = x,y,X-x,Y-y
					sdl.fillRect(windowsurface,rect,255)
					
					-- We also collect the hand blobs to paint later
					vision.rms(b3,l)
				end 
			end
		elseif not ok then
			print(c2,x,X,y,X*0.7)
		end
	end
